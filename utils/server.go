...
func NewServer() (*Server, error) {
	...
	server.Config.RateLimit = Getenv("RATE_LIMIT", "5")

	server.Config.RedisHost = Getenv("REDIS_HOST", "redis")
	server.Config.RedisPassword = Getenv("REDIS_PASSWORD", "!nujek123")
	server.Config.RedisDB = Getenv("REDIS_DB", "0")

	server.Config.MySQLHost = Getenv("MYSQL_HOST", "sing.nujek.id")
	server.Config.MySQLPort = Getenv("MYSQL_PORT", "3306")
	server.Config.MySQLUsername = Getenv("MYSQL_USERNAME", "nujek")
	server.Config.MySQLPassword = Getenv("MYSQL_PASSWORD", "nuj3kj4l4n")
	server.Config.MySQLDatabaseName = Getenv("MYSQL_DATABASE", "nujekappdev")
